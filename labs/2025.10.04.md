...
int find_max(int count, ...) {
  ...
  va_list args;                 // список параметров
  va_start(args, count);        // инициализация коллекции параметров
  
  int max = va_arg(args, int);  // получение 1-го параметра

  for (size_t i = 0; i != count - 1; ++i) {
    int current = va_arg(args, int);  // получение очередного параметра
    if (current > max) {
      max = current;
    }
  }
  
  va_end(args);  // освобождение стека коллекции параметров
